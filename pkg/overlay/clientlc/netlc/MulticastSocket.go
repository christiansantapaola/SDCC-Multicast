...
func (socket *MulticastSocket) Send(ctx context.Context, message *api.MessageLC) error {
	socket.mutex.Lock()
	defer socket.mutex.Unlock()
	return socket.sender.Send(ctx, message)
}

func (socket *MulticastSocket) TrySend(ctx context.Context, message *api.MessageLC) error {
	socket.mutex.Lock()
	defer socket.mutex.Unlock()
...
func (socket *MulticastSocket) Recv(ctx context.Context) (*api.MessageLC, error) {
	for {
...
func (socket *MulticastSocket) NonBlockingRecv() *api.MessageLC {
